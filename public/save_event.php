...
$start = $payload['start'] ?? '';
$end = $payload['end'] ?? '';
if (!$title || !$start || !$end) { http_response_code(422); echo json_encode(['error'=>'All fields are required']); exit; }
...
// $startUtc = (new DateTime($start))->format('Y-m-d H:i:s');
// $endUtc   = (new DateTime($end))->format('Y-m-d H:i:s');
$timezone = new DateTimeZone('Asia/Karachi'); // Set to Pakistan Standard Time (Islamabad)
$startUtc = (new DateTime($start, $timezone))->setTimezone(new DateTimeZone('UTC'))->format('Y-m-d H:i:s');
$endUtc   = (new DateTime($end, $timezone))->setTimezone(new DateTimeZone('UTC'))->format('Y-m-d H:i:s');
if ($endUtc <= $startUtc) { http_response_code(422); echo json_encode(['error'=>'End time must be after start time']); exit; }
# Overlap check: any event where new.start < existing.end AND new.end > existing.start
